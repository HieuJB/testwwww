        ...
        <div class='play'>
            <span class="new">
                <div class="playerevent">
                    ...
                    <div class="events">
                        <template v-for="(incident, index) in player.incidents" :key="incident.time">
                            <BaseImage v-if="incident.type"
                            :alt="$t(getIncidentAlt(incident.type))"
                            :src="getIncidentImage(incident.type)"
                            :class="getClassImage(index + 1)"/>
                        </template>
                        <i class="sr icon RB" v-if="player.rating">{{ player.rating }}</i>
                    </div>
                </div>
                <div class='name'>
