    ...
    >
      <div class="select-box__value">
        ...
        >
          <BaseImage
            :src="
              compActive?.country_id !== 1
              ? getLiveScoreImage(compActive?.logo_o, 'country') + '!h20'
              : compActive.logo_league_o
            "
            :alt="compActive?.country_id"
          />
          <div>
            {{ compActive?.country_name }}
          </div>
        </div>
      </div>
      <BaseImage
    ...
    <ul v-show="isShow" class="select-box__list">
      <div class="select-box__list-input">
        <input
          ...
          :placeholder="translate('Enter Country Name team')"
        />
      </div>
      <div class="select-box__list-comp">
        <li
          ...
          :key="index"
          :class="{
            'item--active' : compActive.country_id === item.country_id
          }"
        >
          <div class="select-box__option" for="0" aria-hidden="true">
            <BaseImage
              :src="
                item.country_id !== 1
                  ? getLiveScoreImage(item?.logo_o, 'country') + '!h20'
                  : item.logo_league_o
              "
              :alt="compActive?.country_id"
            />
...
const listComp = computed(() => {
  if (search.value) {
    return info.value.filter((item) =>
      item.country_name?.toLowerCase().includes(search.value.toLowerCase())
    );
  }
