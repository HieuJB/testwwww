        ...
        <div class="rankbox">
          <table class="eTable adaptMt" id="etable-header" width="25%" cellspacing="0" cellpadding="0" style="float: left; min-width: 135px">
            ...
            <table class="eTable adaptMt w-100" cellspacing="0" cellpadding="0" width="100%">
              <tbody>
                <tr class="tr-title" >
                    <th colspan="4" v-show="showColumn.first"><b>{{ $t('Init') }}</b></th>
                    <th colspan="4" v-show="showColumn.second"><b>{{ $t('Live') }}</b></th>
                ...
                >
                <tr name="oddsTr" :class="index % 2 !== 0 ? 'tb-bgcolor1' : 'tb-bgcolor1'">
                  <td v-show="showColumn.first" width="6%"><span data-o="">{{ item?.early_handicap }}</span></td>
                  <td v-show="showColumn.first" width="6%"><span data-o="">{{ item?.early_home }}</span></td>
                  <td v-show="showColumn.first" width="6%"><span data-o="">{{ item?.early_draw }}</span></td>
                  <td v-show="showColumn.first" width="6%" class="rb"><span data-o="">{{ item?.early_away }}</span></td>
                  <td v-show="showColumn.second" width="6%"><span data-o="">{{ item?.live_handicap }}</span></td>
                  <td v-show="showColumn.second" width="6%"><span data-o="" class="">{{ item?.live_home }}</span></td>
                  <td v-show="showColumn.second" width="6%"><span data-o="" class="">{{ item?.live_draw }}</span></td>
                  <td v-show="showColumn.second" width="6%" class="rb"><span data-o="" class="">{{ item?.live_away }}</span></td>
                  <td width="3%">
                    <span class="odd_a" @click="openModalDetailChange(item?.i_company_id, oddCompanysList?.[item?.i_company_id]?.name)">{{ $t('History') }}</span>
                  </td>
                </tr>
              </template>
              </tbody>
      ...
      <div class="modal-dialog modal-dialog-centered container">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            >
              <div class="popinfo">
                <PopupOddsChange :odd-comp-id="oddCompanyId" :match-id="matchIdSlug" :section="'handicapDetail'" :odd-companys-list="oddCompanys" :refresh-time="refreshTime"/>
              </div>
            </div>
          </div>
        </div>
...
const fetchOdds = async (matchIdSlug) => {
  await useApiLiveScore(API_ROUTERS.LIVESCORE.ODDS_EUROHADICAP + '?match_id=' + matchIdSlug).then(async resData => {
      oddCompanys.value = resData?.data
  });
};
